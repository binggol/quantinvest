...
@app.route("/api/tradingagents/analyze")
def api_ta_analyze():
    ...
    tickers = [t.strip() for t in raw.split(",") if t.strip()][:10]   # 上限 10 只, 防滥用
    if not tickers:
        return jsonify({"ok": False, "message": "没有选中股票"})
    date = (request.args.get("date", "") or "").strip()
    try:
        TA_REQUEST.parent.mkdir(parents=True, exist_ok=True)
        TA_REQUEST.write_text(json.dumps(
            {"requested_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "tickers": tickers, "date": date},
            ensure_ascii=False), encoding="utf-8")
    except Exception as e:
        return jsonify({"ok": False, "message": f"写请求失败: {e}"})
...
@app.route("/api/strategy/request")
def api_strategy_request():
    ...
    try:
        RDAGENT_REQUEST.parent.mkdir(parents=True, exist_ok=True)
        RDAGENT_REQUEST.write_text(json.dumps(
            {"requested_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "strategy_bt": True,
             "model": model, "batch": batch, "hold_days": hold, "topn": topn, "rt_cost": cost},
            ensure_ascii=False), encoding="utf-8")
    except Exception as e:
        return jsonify({"ok": False, "message": f"写请求失败: {e}"})
    bmsg = f" [批次 {batch}]" if batch else ""
...
def api_advisor_request():
    ...
    try:
        RDAGENT_REQUEST.parent.mkdir(parents=True, exist_ok=True)
        RDAGENT_REQUEST.write_text(json.dumps(
            {"requested_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "regime_adv": True},
            ensure_ascii=False), encoding="utf-8")
    except Exception as e:
        return jsonify({"ok": False, "message": f"写请求失败: {e}"})
...
def _save_watchlist(codes: list[str]):
    ...
    WATCHLIST_JSON.parent.mkdir(parents=True, exist_ok=True)
...
def _meta_for_codes(codes: list[str]) -> dict[str, dict]:
    ...
    try:
        ...
        conn.row_factory = sqlite3.Row
        qs = ",".join("?" * len(codes))
        cur = conn.execute(f"SELECT code, name, industry FROM stock_meta WHERE code IN ({qs})", codes)
        for r in cur.fetchall():
            out[r["code"]] = {"name": r["name"], "industry": r["industry"]}
        conn.close()
    except Exception as e:
...
def api_watchlist_add():
    ...
    return jsonify({"ok": True, "count": len(codes), "watching": code in codes})
